...
The training involves finding the regression line which fits the given training data the best, meaning that the squared sum of the errors made is at its minimum. This minimum is found by using the gradient descent algorithm. The regression line is described by $estimated price(km) = Θ_0 + Θ_1 * km$
